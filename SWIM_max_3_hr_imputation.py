...
# for swimmer in range(1, 10): #range is set manually, needed to exclude swimmer 10 in this case
for swimmer in range(11, 13): #range is set manually, needed to exclude swimmer 10 in this case
    ...
    df.drop(columns=['imputed', 'MVPA', 'light', 'sedentary', 'sleep', 'MET'], inplace=True, axis=1) #eliminates the columns not needed in nparact (everything except for data/time and acceleration)
    df.to_csv(r"C:\Users\lab\Desktop\SWIMAccelerometerData\Less_than_3_hr_imputed\SWIM" +
              str(swimmer) + '-less_than_3hr_imputed--test.csv', index=False, header=False)
pass
